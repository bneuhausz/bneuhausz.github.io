...
<p>So this new <code>submit</code> function accepts a <code>Field</code> and an <code>action</code>. The <code>Field</code> will be our entire form and <code>action</code> will be a mock HTTP request. With this, we will demonstrate how you could validate the entire form on the server side after submitting, but the errors returned by our <code>action</code> will be applied automatically to the correct sub-field.</p><p>Let&#39;s take a look at our code and it will make more sense:</p><pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#E1E4E8">@</span><span style="color:#B392F0">Component</span><span style="color:#E1E4E8">({</span></span>
...
<span class="line"><span style="color:#E1E4E8">  imports: [MatFormFieldModule, MatInputModule, MatButtonModule, Control],</span></span>
...
<span class="line"><span style="color:#E1E4E8">        &#x3C;</span><span style="color:#85E89D">input</span><span style="color:#B392F0"> matInput</span><span style="color:#B392F0"> [control]</span><span style="color:#E1E4E8">=</span><span style="color:#9ECBFF">"f.name"</span><span style="color:#E1E4E8"> /></span></span>
...
<span class="line"><span style="color:#E1E4E8">  }</span></span>
<span class="line"></span>
<span class="line"><span style="color:#B392F0">  mockHttpRequest</span><span style="color:#E1E4E8">(</span><span style="color:#FFAB70">form</span><span style="color:#F97583">:</span><span style="color:#B392F0"> Field</span><span style="color:#E1E4E8">&#x3C;{ </span><span style="color:#FFAB70">name</span><span style="color:#F97583">:</span><span style="color:#79B8FF"> string</span><span style="color:#E1E4E8"> }>) {</span></span>
...
<span class="line"><span style="color:#E1E4E8">          field: form.name,</span></span>
...
<p>We added a <code>(click)</code> listener to our button, which will invoke our <code>submitForm</code> function when it fires.</p><p>Then, we create our <code>mockHttpRequest</code>, which accepts our <code>Field</code> as a parameter and it will simply return a Promise, which is enough for demonstration purposes. It&#39;s important to note that our function accepts the <code>Field</code> itself, not the value inside. If our validation criteria passes, we return undefined. If it doesn&#39;t, then we have to return our error in a specific format.</p><p>We are returning an array of errors. The <code>kind</code> and <code>message</code> are pretty straightforward and we looked at those in the <a href="https://bneuhausz.dev/blog/angular-signal-forms-are-out">first post about the new signal forms.</a></p><p>The <code>field</code> property is new though. We pass the specific sub-field we want this error to be applied to. Again, we pass the field and subfield without invoking anything, so make sure it is <code>form.name</code> and not <code>form.name()</code> or anything like that. This will be used by <code>submit</code> in the background to identify which sub-field the error has to be applied to.</p><p>Now all we have left to do is to call <code>submit</code> in our <code>submitForm</code> method and pass the form and our action. Assuming we&#39;ve done everything right, after we click our button, if the validation criteria in <code>mockHttpRequest</code> does not pass, our validation kind and message will show up right under our input, inside <code>mat-form-field</code>, which only shows up if specifically the <code>name</code> input is invalid, proving that this works as we intended.</p><h2 id="documentation-issue">Documentation issue</h2>
<p>One thing I should point out, is that at the time of writing this article, the documentation is not consistent with the actual functionality.</p><p><img src="/images/angular-signal-forms/signal_forms_doc_error.avif" alt="windows notification"></p><p>Issues are expected, as signal forms are very much experimental still, but make sure to pay attention, because the example from the documentation above tells us to return the error in a different format that will not work, as the actual definition of the ValidationError interface is this:</p><pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#6A737D">/**</span></span>
...
<span class="line"><span style="color:#E1E4E8">}</span></span></code></pre>
<p>I&#39;m sure these little errors will be fixed fairly quickly, but even with these being around, the new Angular signal forms are a huge step in the right direction. I honestly can&#39;t wait to actually start using these features in production.</p><p><a href="https://bneuhausz.dev/blog/angular-signal-forms-advanced">The next issue about more advanced usages of Angular signal forms is out!</a></p>`;export{s as default};
